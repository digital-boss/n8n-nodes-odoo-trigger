	...
	methods = {
		loadOptions: {
			async getModels(this: ILoadOptionsFunctions): Promise<INodePropertyOptions[]> {
				...
				const options = (response as any).result.map(
	...
	webhookMethods = {
		default: {
			async checkExists(this: IHookFunctions): Promise<boolean> {
				...
				const response = await odooGet.call(
					this,
					db,
					userID,
					password,
					...
					'get',
					url,
					webhookData.webhookId?.toString() as string,
				);

				const exists = (response as any).result.length > 0;
			...
			async create(this: IHookFunctions): Promise<boolean> {
				...
				webhookData.webhookId = (response as any).result;
			...
			async delete(this: IHookFunctions): Promise<boolean> {
				...
				const response = await odooDelete.call(
					this,
					db,
					userID,
					password,
